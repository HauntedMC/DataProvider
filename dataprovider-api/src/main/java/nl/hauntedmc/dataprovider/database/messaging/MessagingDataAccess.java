...
/**
 * Defines messaging-related methods (for example event publishing and subscription)
...
public interface MessagingDataAccess extends DataAccess {
    ...
    /**
     * Subscribe to messages of a given type on the destination. The returned logical handle remains valid
     * when a backend replaces the physical listener connection.
     */
    ...
    /** Current logical subscription diagnostics exposed by implementations that support them. */
    default List<SubscriptionSnapshot> subscriptions() {
        return List.of();
    }
